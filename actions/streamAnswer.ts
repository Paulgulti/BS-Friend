...
export async function streamAnswer(input: string, clientPrevMessages?: { role: string; content: string }[]) {
    const { userId } = await auth();
    if (!userId) throw new Error('Unauthorized');

    // Prefer client-provided history to avoid extra Redis GET per message
    let prevMessages: { role: string; content: string }[] | [] = Array.isArray(clientPrevMessages)
        ? clientPrevMessages
        : [];
...
Rules:
- Only answer questions related to the Bible or Christian spirituality.
- Cite scripture where possible.
- If the question is not related to the Bible, politely refuse to answer.
- Suggest one or two related follow-up questions.

Background notes (NOT provided by the user): 
These are relevant verses that may help you answer. Use them naturally if helpful, 
...
${context}`
    ...
    const stream = await groq.chat.completions.create({
        model: 'llama-3.1-8b-instant',
        messages: [systemPrompt, ...newMessages] as any,
        temperature: 0.2,
        stream: true,
    });
