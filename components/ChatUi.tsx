...
import { logChatClosed } from '@/actions/logChatLifecycle';
import { streamAnswer } from '@/actions/streamAnswer';
...
export default function ChatUI({ initialHistory }: { initialHistory: ChatMessage[] }) {
  ...
  async function handleSend(e: React.FormEvent<HTMLFormElement>) {
    ...
    const stream = await streamAnswer(input, messages); // send prior history only to avoid duplication
    const reader = stream.getReader();
    const decoder = new TextDecoder();
    let assistantText = '';
    ...
    <div>
      ...
      {!isOpen && (
        <button
          onClick={async () => {
            setIsOpen(true);
            // Open: load history lazily (also increments opens server-side)
          ...
          <div className="flex-1 overflow-y-auto p-3 bg-gray-50 max-h-[300px] rounded-lg border mt-3">
            {messages.length < 1 && (
              <div className='text-center'>
                <p className='text-gray-700'> Hi, I am your AI BS Friend. <br/> <span>Let's study the bible together ✍</span></p>
              </div>
            )}
            {messages.map((msg, idx) => (
              <div
